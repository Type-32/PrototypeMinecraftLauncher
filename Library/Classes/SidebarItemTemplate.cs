...
namespace ProtoLauncher.Library.Classes;

public class SidebarItemTemplate
{
    ...
    public SidebarItemTemplate(Type type, string sidebarLabel, string iconKey)
    {
        ModelType = type;
        Label = sidebarLabel;

        // Try to find the resource.
        var resourceFound = Application.Current!.TryFindResource(iconKey, out var res);
        if (!resourceFound || res as StreamGeometry == null)
        {
            // Log the issue.
            var issue = !resourceFound ? "Resource not found." : "Resource is not a StreamGeometry.";
            Console.WriteLine($"{issue} IconKey: {iconKey}, Type: {res?.GetType().FullName}");

            // Use the default geometry.
            res = StreamGeometry.Parse(StreamGeometryNotFound);
        }
